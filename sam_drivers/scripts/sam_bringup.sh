...
# tmux send-keys "mon launch sam_camera_config sam_enhance.launch sim:=false sensor_id:=$SENSOR_ID --name=$(tmux display-message -p 'p#I_#W') --no-start" C-m
tmux send-keys "mon launch sam_camera_config sam_cameras.launch sim:=false --name=$(tmux display-message -p 'p#I_#W') --no-start" C-m
