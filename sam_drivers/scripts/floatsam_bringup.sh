...
tmux send-keys "roslaunch sam_drivers sam_gui.launch rosbridge_ip:=$FLOATSAM_IP namespace:=sam --wait" C-m

tmux select-window -t $SESSION:2
tmux send-keys "mon launch sam_drivers floatsam_core.launch utm_zone:=$UTM_ZONE utm_band:=$UTM_BAND --name=$(tmux display-message -p 'p#I_#W') --no-start" C-m
...
tmux send-keys "mon launch bt_mission mission.launch utm_zone:=$UTM_ZONE utm_band:=$UTM_BAND neptus_addr:=$NEPTUS_IP bridge_addr:=$FLOATSAM_IP bridge_port:=$BRIDGE_PORT --name=$(tmux display-message -p 'p#I_#W') --no-start" C-m
